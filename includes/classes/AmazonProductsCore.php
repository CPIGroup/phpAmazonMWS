...
abstract class AmazonProductsCore extends AmazonCore
{
    ...
    protected function parseXML($xml)
    {
        ...
        foreach ($xml->children() as $xKey => $x) {
            if ($x->getName() == 'ResponseMetadata') {
                continue;
            }
            $temp = (array) $x->attributes();
            $identifier = $temp['@attributes']['Id'] ?? $temp['@attributes']['ASIN'] ?? $xKey;
            ...
            if (isset($x->Products)) {
                foreach ($x->Products->children() as $z) {
                    $this->productList[$identifier] = $this->createProduct($z);
                    if (isset($temp['@attributes'])) {
                        $this->productList[$identifier]->data['Identifiers']['Request'] = $temp['@attributes'];
                    }
                }
                continue;
            }
            ...
            if (in_array($x->getName(), $resultTags)) {
                $this->productList[$identifier] = $this->createProduct($x);
                continue;
            }
     ...
     */
    protected function createProduct(SimpleXMLElement $element): AmazonProduct
    {
        return new AmazonProduct($this->storeName, $element, $this->mockMode, $this->mockFiles, $this->config);
    }
     ...
     * @param string|int $num [optional] <p>List index to retrieve the value from</p>
