<?php namespace CPIGroup;
    ...
    abstract class AmazonCore
    {
         ...
         *                             If there is only one store defined in the config file, this parameter is not
         *                             necessary. If there is more than one store and this is not set to a valid name,
         *                             none of these objects will work.</p>
         ...
         *                                    put it into an array. Integers can also be given, for use in
         *                                    <i>fetchMockResponse</i>. These numbers should only be response codes,
         *                                    such as <b>200</b> or <b>404</b>.</p>
         */
        ...
        protected function fetchMockFile( $load = true )
        {
            ...
            if (strpos( $this->mockFiles[ $this->mockIndex ], '/' ) === 0
                || strpos( $this->mockFiles[ $this->mockIndex ], '..' ) === 0
            ) {
  ...
  <RequestID>123</RequestID>
        ...
        protected function checkResponse( $r )
        {
            ...
            if ($r[ 'code' ] == 200) {
                ...
                $this->log( "Bad Response! " . $r[ 'code' ] . " " . $r[ 'error' ] . ": " . $xml->Code . " - "
                            . $xml->Message, 'Urgent' );
        ...
        protected function log( $msg, $level = 'Info' )
        {
            ...
            if ($msg != false) {
                ...
                if (isset( $backtrace ) && isset( $backtrace[ 1 ] ) && isset( $backtrace[ 1 ][ 'file' ] )
        ...
        protected function genQuery()
        {
            ...
            if (array_key_exists( $this->storeName, $store )
                && array_key_exists( 'secretKey', $store[ $this->storeName ] )
            ) {
        ...
        function fetchURL( $url, $param )
        {
            ...
            $data = preg_match( "/meta http-equiv=.refresh. +content=.[0-9]*;url=([^'\"]*)/i", $return[ 'body' ],
                $matches );
            if (!empty( $matches )) {
            ...
            if ($return[ 'code' ] == '200' || $return[ 'code' ] == '302') {
                ...
                $return[ 'error' ] = ( ( $return[ 'answer' ] and
                                         $return[ 'answer' ] != 'OK' ) ? $return[ 'answer' ] : 'Something wrong!' );
                $return[ 'ok' ]    = 0;
            }
